...
function FinancialAnalytics() {
  ...
  const getFinancialValue = useCallback((item) => {
    ...
    const rawValue = isProject ? item.order_value ?? item.orderValue ?? item.orderValue : item.quote_amount ?? item.quoteAmount ?? item.quote_amount
  ...
  const buildBreakdown = useCallback((items, dimension) => {
    ...
    items.forEach((item) => {
      ...
      sums[key] = (sums[key] || 0) + getFinancialValue(item)
    })
  ...
  }, [buildBreakdown, CATEGORIES, drillLevel, filterForDrill, filteredData, matchCategory, selectedCategory, selectedCenter, selectedGroup])
    ...
    const valuePctLabelsPlugin = {
      ...
      afterDatasetsDraw: (chart) => {
        ...
        const drawTwoLine = (x, y, line1, line2) => {
          ...
          // Treemap tiles are colored => white text; other charts on white => dark text
          canvasCtx.fillStyle = isTreemap ? '#ffffff' : '#1f2937'
        ...
        elements.forEach((el, idx) => {
          ...
          const labelValue = Number.isFinite(v) ? formatInCrore(v) : String(v)
      ...
      ? {
          ...
          labels: {
            ...
            formatter: (ctx) => {
              ...
              return [name, `${formatInCrore(v)} (${pct.toFixed(1)}%)`]
            },
          },
        }
      : {
          ...
          : {
              y: {
                ...
                ticks: {
                  color: '#374151',
                  font: { size: 12 },
                  callback: (value) => formatInCrore(Number(value)),
                },
                title: { display: true, text: 'Amount (cr)', color: '#374151' },
              },
              x: {
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: (context) => {
                ...
                const pct = totalValue > 0 ? (v / totalValue) * 100 : 0
                const displayValue = Number.isFinite(v) ? formatInCrore(v) : String(v)
                return `${name}: ${displayValue} (${pct.toFixed(1)}%)`
              },
            },
          },
        },
