...
const getProjectCode = (projectNumber) => {
  ...
  // Extract prefix before the first number or dash
...
// Project name modal state - removed, now using chart drill-down

// Function to handle project code click
const handleProjectCodeClick = (projectCode) => {
...
const TABLE_FIELDS = [
  ...
  { 
    ...
    width: 120,
    render: (text, record) => {
      const code = getProjectCode(record.project_number)
      const activity = record.activity || ''
      return code ? (
        <Tag 
          color="blue" 
          style={{ cursor: 'pointer' }}
          onClick={() => handleProjectCodeClick(activity)}
        >
          {code}
        </Tag>
      ) : '-'
...
const [enquiryDateRange, setEnquiryDateRange] = useState(null)
  ...
  const chartData = useMemo(() => {
    ...
    if (drillLevel === 'project_code_detail') {
      const filteredItems = items.filter((item) => getProjectCode(item.project_number) === selectedProjectCode)
      return {
    ...
    return {
      ...buildBreakdown(items, 'project_code'),
      title: `${CHART_CATEGORIES.find((c) => c.key === selectedCategory)?.label || 'All'} by Project Code`,
      dimension: 'project_code',
    }
  }, [buildBreakdown, CHART_CATEGORIES, chartMetric, drillLevel, filterForDrill, filteredData, matchCategory, selectedCategory, selectedProjectCode])
  ...
  const handleChartClick = useCallback(
    (label) => {
      ...
      if (dimension === 'activity') {
        // Activity level - no further drilling
        return
      }
    },
  ...
  const handleRemarksSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        to: remarksTarget === 'admin' ? 'admin' : 'admin',
      ...
      try {
        ...
        if (rawUser) {
          ...
          if (parsedUser && parsedUser.name) {
            ...
            const queryString = parsedUser.role?.toLowerCase() === 'scientist' ? '?user_role=scientist' : ''
            url = `${API_BASE_URL}/proposals/by-name/${encodedName}${queryString}`
          }
        }
      } catch (storageError) {
          ...
          try {
            ...
            if (queriesResponse.ok) {
              ...
              return { 
                ...
                project_code: getProjectCode(proposal.project_number)
              }
            }
          } catch (error) {
    ...
    try {
      ...
      if (rawUser) {
        ...
        if (name) {
          ...
          const queryString = parsedUser.role?.toLowerCase() === 'scientist' ? '?user_role=scientist' : ''
          const url = `${API_BASE_URL}/proposals/by-name/${encodedName}${queryString}`
  ...
  const fetchUnacknowledgedProposals = async () => {
    ...
    try {
      ...
      // Add project_code to each item
      normalized = normalized.map(item => ({
        ...item,
        project_code: getProjectCode(item.project_number)
      }))
  ...
  const itemCode = (item.project_code || '').toString().trim().toLowerCase()
  const filterCode = coordinatorFilter.toString().trim().toLowerCase()
      ...
      return [
        ...
        },
        {
          ...
          render: (value, record) => {
            ...
            const activity = record.activity || ''
            return code ? (
              <Tooltip title={activity} placement="topLeft">
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} md={4}>
                    <Select placeholder="Project Code" value={coordinatorFilter} onChange={setCoordinatorFilter} allowClear size="large" style={{ width: '100%' }}>
          ...
          <div style={{ maxHeight: '65vh', overflowY: 'auto' }} className="space-y-4">
            ...
            <Card title="Project Details" size="small" className="bg-green-50">
              <Descriptions bordered size="small" column={{ xs: 1, sm: 2 }}>
                ...
                <Descriptions.Item label="Project Code">{getProjectCode(selectedRecord?.project_number) || '-'}</Descriptions.Item>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input 
              value={remarksTarget === 'admin' ? 'admin' : 'admin'} 
              disabled 
              className="w-full" 
            />
          </div>
